...
(function () {
    ...
    const { getRegister } = systemPrototype;
    systemPrototype.getRegister = function () {
        const register = getRegister.call(this);
        // if it is an actual System.register call, then its ESM
        // -> dont add named exports
        if (!register || register[1] === lastRegisterDeclare || register[1].length === 0) return register;
        ...
        // hook the _export function to note the default export
            let defaultExport; let 
                hasDefaultExport = false;
            const declaration = registerDeclare.call(this, (name, value) => {
                if (typeof name === 'object' && name.__useDefault) defaultExport = name.default, hasDefaultExport = true;
