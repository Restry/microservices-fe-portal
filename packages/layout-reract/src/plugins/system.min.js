...
(function () {
    ...
    function resolveAndComposePackages(packages, outPackages, baseUrl, parentMap, parentUrl) {
        for (const p in packages) {
            ...
            if (!mapped) { targetWarning(p, rhs, 'bare specifier did not resolve'); } else { outPackages[p] = mapped; }
        }
    }
    ...
    systemJSPrototype.import = function (id, parentUrl) {
        ...
        return Promise.resolve(loader.prepareImport())
            .then(() => loader.resolve(id, parentUrl))
            .then(id => {
    ...
    function getOrCreateLoad(loader, id, firstParentUrl) {
        ...
        instantiatePromise = instantiatePromise.catch(err => {
    ...
    let importMap = { imports: {}, scopes: {} }; let 
        importMapPromise;
  
    if (hasDocument) {
        Array.prototype.forEach.call(document.querySelectorAll('script[type="systemjs-importmap"][src]'), script => {
            script._j = fetch(script.src).then(res => res.json());
        });
    }
    ...
    systemJSPrototype.prepareImport = function () {
        if (!importMapPromise) {
            ...
            if (hasDocument) {
                Array.prototype.forEach.call(document.querySelectorAll('script[type="systemjs-importmap"]'), script => {
                    importMapPromise = importMapPromise.then(() => (script._j || script.src && fetch(script.src).then(resp => resp.json()) || Promise.resolve(JSON.parse(script.innerHTML)))
                        .then(json => {
    ...
    function loadScriptModules() {
        document.querySelectorAll('script[type=systemjs-module]').forEach(script => {
            if (script.src) {
    ...
    (function (global) {
        ...
        let firstGlobalProp; let secondGlobalProp; let 
            lastGlobalProp;
    ...
    (function () {
        ...
        const { instantiate } = systemPrototype;
        ...
        systemPrototype.instantiate = function (url, parent) {
            ...
            switch (ext) {
            case '.css':
                return loadDynamicModule((_export, source) => {
            ...
            case '.html':
                return getSourceRes().then(res => maybeJSFallback(res) || loadError('\'.html\' modules not implemented'));
            case '.json':
                return loadDynamicModule((_export, source) => {
                    _export('default', JSON.parse(source));
                });
            case '.wasm':
                return getSourceRes().then(res => maybeJSFallback(res) ||
                  (WebAssembly.compileStreaming ? WebAssembly.compileStreaming(res) : res.arrayBuffer().then(WebAssembly.compile)))
                    .then(module => {
                        ...
                        if (WebAssembly.Module.imports) {
                            WebAssembly.Module.imports(module).forEach(impt => {
            ...
            function loadDynamicModule(createExec) {
                return getSourceRes().then(res => maybeJSFallback(res) || res.text().then(source => [[], function (_export) {
    ...
    systemJSPrototype.entries = function () {
        const loader = this; const 
            keys = Object.keys(loader[REGISTRY]);
        let index = 0; let ns; let 
            key;
        const result = {
            next() {
                while (
